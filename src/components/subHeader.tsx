...
const SubHeader : React.FC<SubHeaderProps> = ({onChangeHandler}) => (
    ...
    <div className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8 flex lg:justify-between md:justify-between flex-col">
      ...
      <div className="inset-y-0 right-0 flex items-center lg:justify-between lg:mt-0 mt-3">
        <label htmlFor="event_type" className="text-sm uppercase font-bold text-gray-500 mx-4">Event Type</label>
        <select id="event_type" name="event_type" onChange={(event: ChangeEvent<HTMLSelectElement>) => {onChangeHandler(event.target.value)}} className="ring-indigo-300 border border-gray-500 h-full py-1 pl-2 pr-7 bg-transparent text-gray-500 sm:text-sm rounded-md capitalize">
            <option value="">All</option>
            {eventTypes.map(eventType => (<option key={eventType} value={eventType} className="capitalize">{removeUnderScore(eventType)}</option>))}
        </select>
        </div>
    </div>
  </header>
